...
define(['application-configuration','lodash'], function (app) {
    app.register.directive('actionAddEditSiteMapping',['customerSettingsFactory','Notify','$filter',
            ...
            return {
                ...
                link:function (scope, el, attrs) {
                    ...
                    function InitializeData(){
                        ...
                        ownClusterName = scope.selectedSite.ClusterName;
                    }
                    ...
                    function validInput(){
                        var valid = true;
                        if(angular.isUndefined(ownClusterName))
                            ownClusterName = "";
                        scope.isExist = false;
                        ...
                        if(scope.mapData.SiteClusterCollection && ownClusterName.toLowerCase().trim() != scope.selectedSite.ClusterName.toLowerCase().trim()){
                    ...
                    scope.searchBtnClick = function(){
                        ...
                        customerSettingsFactory.GetDistinctSiteList(searchData);
                    };
                    scope.$on(Notify.CUSTOMER_SETTINGS_DISTINCT_SITE_DATA_READY, function (event) {
                        scope.distinctSiteList = [];
                        if(scope.isEdit && scope.selectedSite.Sites){
                            _.forEach(scope.selectedSite.Sites,function(f){
                                scope.distinctSiteList.push({isSelected:true,ClusterId: f.ClusterId,SiteId: f.SiteId,SiteName: f.SiteName,GroupId: f.GroupId});
                            });
...
//                                });
                        ...
                        var distinctList = customerSettingsFactory.DistinctSiteList();
                        if(distinctList && distinctList.length > 0){
                            _.forEach(distinctList,function(f){
                                scope.distinctSiteList.push({isSelected:false,ClusterId: null,SiteId:null,SiteName: f,GroupId: null});
                            });
                        }
                        scope.distinctSiteList = _.sortBy(scope.distinctSiteList, function(x) {return x.SiteName.toLowerCase();});
                    });
                    scope.item_checked = function(item){
