...
define(['application-configuration', 'd3','topojson','dataFactory','CreateViewDataFactory', 'comparatorFactory'], function (app,d3,topojson) {
    ...
    app.register.controller('mapViewController', ['$document','$rootScope','$scope','$timeout','$compile', 'dataFactory','Notify','CreateViewDataFactory','comparatorFactory','serviceAreaScoreData', 'fullScorecardDataFactory', 'attributeScoreSummaryData', 'blockUI',
        ...
        $scope.$on(Notify.DATA_READY, function () {
            $scope.summaryPopupCloseHandler($scope.event);
            initializeCountryViewData();
            updateMapView();
            if(dataFactory.IsResetFilterOrView()){
                reset();
        ...
        $scope.$on(Notify.LEFT_MENU_FOLD_BUTTON_CLICK, function(event, args){
            isLeftMenuExist = !isLeftMenuExist;
            if(args.activeTab === 'mapview') {
                ...
                if (isSummaryPopupExpanded) {
                    $scope.ngClassVar = isLeftMenuExist ? "default-expanded-view" : "full-screen-expanded-view";
                } else {
                    $scope.ngClassVar = isLeftMenuExist ? "default-collapsed-view" : "full-screen-collapsed-view";
                }
            }
        });
        ...
        $scope.expandCollapseClickHandler = function(isExpanded){
            isSummaryPopupExpanded = isExpanded;
            if(isSummaryPopupExpanded) {
                $scope.ngClassVar = isLeftMenuExist ? "default-expanded-view expand-details" : "full-screen-expanded-view expand-details";
            }else{
                $scope.ngClassVar = isLeftMenuExist ? "default-collapsed-view collapse-details": "full-screen-collapsed-view collapse-details";
            }
        }
        //Private Methods
        ...
        function clickHandler(selectedCountry){
            ...
            if(selectedCountryViewScoreData){
                if(!document.getElementById('map-view-summary')) {
                    $scope.$apply(function () {
                        ...
                        angular.element(document.getElementById('mapViewContainer')).append($compile("<div id='map-view-summary' ng-class='ngClassVar' ng-show='isNeedToShowMapSummary' class='show-popup'>" +
                            ...
                            "control='countryPopUp' attribute-data='attributeData' on-summary-popup-close='summaryPopupCloseHandler($event)' on-expand-collapse-click='expandCollapseClickHandler(isExpanded)'></action-attribute-score-summary></div>")($scope));
