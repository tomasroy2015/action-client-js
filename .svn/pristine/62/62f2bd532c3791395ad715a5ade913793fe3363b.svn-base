"use strict";
define(['application-configuration','accountFactory','lodash','datasetSelectionFactory'], function (app) {
    app.register.controller('customerSettingsController', ['$scope','$rootScope','$modalInstance','accountFactory','$timeout',
        'customerSettingsFactory','UserType','Notify','datasetSelectionFactory',
        function ($scope,$rootScope,$modalInstance,accountFactory,$timeout,customerSettingsFactory,UserType,Notify,datasetSelectionFactory) {
            $scope.selectedCustomer = {};
            $scope.currentuser ={};
            angular.copy(accountFactory.UserData(),$scope.currentuser);
            $scope.sessionID = $scope.currentuser.SessionId;
            customerSettingsFactory.IsDatasetInfoChanged (false);
            var selectedCustomerDataSet = {};
            selectedCustomerDataSet = datasetSelectionFactory.selectedCustomer;
            /*
             CustomerId: "AEGN"
             CustomerName: "Aegon"
             */
 ...
 */
            ...
            $scope.setSelectedCustomer2=function(){

                var customer = {};
                if(angular.isUndefined(selectedCustomerDataSet)){

                    customer = $scope.currentuser.UserToCustomers[0];
                    customerSettingsFactory.SetSelectedCustomer(customer);
                }
                else{

                    customer = _.filter($scope.currentuser.UserToCustomers,function(item){
                       return item.CustomerId==selectedCustomerDataSet.CustomerId
                    })[0];
                    customerSettingsFactory.SetSelectedCustomer(customer);
                }
                $scope.selectedCustomer= getCurrentCustomerFromFactory(customer);
            ...
            $rootScope.$on(Notify.CLOSE_SETTINGS_WINDOW, function(event){
                if($modalInstance)
                    $modalInstance.dismiss('cancel');
            });
            $scope.cancel = function () {
                if(selectedCustomerDataSet && customerSettingsFactory.DatasetInfoChangedStatus()){
                    $rootScope.$broadcast(Notify.RELOAD_DATASET_FOR_CUSTOMER_SETTINGS_CHANGED);
//                    if(selectedCustomerDataSet.CustomerId === customerSettingsFactory.GetSelectedCustomer().Others.CustomerId){
//                     $rootScope.$broadcast(Notify.RELOAD_DATASET_FOR_CUSTOMER_SETTINGS_CHANGED);
//                    }
                }
                $modalInstance.dismiss('cancel');
