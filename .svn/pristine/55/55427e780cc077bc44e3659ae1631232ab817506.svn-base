(function(angular, factory) {
     ...
     * @url https://github.com/esvit/ngtable/
        ...
        function ($compile, $q, $parse) {
            ...
            return {
                ...
                compile: function (element) {
                    ...
                    // IE 8 fix :not(.ngtable-group) selector
                    angular.forEach(angular.element(element.find('tr')), function (tr) {
                        tr = angular.element(tr);
                        if (!tr.hasClass('ngtable-group') && !row) {
                            row = tr;
                        }
                    });
                    if (!row) {
                    ...
                    return function (scope, element, attrs) {
                        ...
                        if (!element.hasClass('ngtable')) {
                            scope.templates = {
                                header: (attrs.templateHeader ? attrs.templateHeader : 'ngtable/header.html'),
                                pagination: (attrs.templatePagination ? attrs.templatePagination : 'ngtable/pager.html')
                            };
    ...
    angular.module('ngTable').run(['$templateCache', function ($templateCache) {
        $templateCache.put('ngtable/filters/select-multiple.html', '<select ng-options="data.id as data.title for data in column.data" ng-disabled="$filterRow.disabled" multiple ng-multiple="true" ng-model="params.filter()[name]" ng-show="filter==\'select-multiple\'" class="filter filter-select-multiple form-control" name="{{column.filterName}}"> </select>');
        $templateCache.put('ngtable/filters/select.html', '<select ng-options="data.id as data.title for data in column.data" ng-disabled="$filterRow.disabled" ng-model="params.filter()[name]" ng-show="filter==\'select\'" class="filter filter-select form-control" name="{{column.filterName}}"> </select>');
        $templateCache.put('ngtable/filters/text.html', '<input type="text" name="{{column.filterName}}" ng-disabled="$filterRow.disabled" ng-model="params.filter()[name]" ng-if="filter==\'text\'" class="input-filter form-control"/>');
        $templateCache.put('ngtable/header.html', '<tr> <th ng-repeat="column in $columns" ng-class="{ \'sortable\': parse(column.sortable), \'sort-asc\': params.sorting()[parse(column.sortable)]==\'asc\', \'sort-desc\': params.sorting()[parse(column.sortable)]==\'desc\' }" ng-click="sortBy(column, $event)" ng-show="column.show(this)" ng-init="template=column.headerTemplateURL(this)" class="header {{column.class}}"> <div ng-if="!template" ng-show="!template" ng-bind="parse(column.title)"></div> <div ng-if="template" ng-show="template" ng-include="template"></div> </th> </tr> <tr ng-show="show_filter" class="ngtable-filters"> <th ng-repeat="column in $columns" ng-show="column.show(this)" class="filter"> <div ng-repeat="(name, filter) in column.filter"> <div ng-if="column.filterTemplateURL" ng-show="column.filterTemplateURL"> <div ng-include="column.filterTemplateURL"></div> </div> <div ng-if="!column.filterTemplateURL" ng-show="!column.filterTemplateURL"> <div ng-include="\'ngtable/filters/\' + filter + \'.html\'"></div> </div> </div> </th> </tr> ');
        var pagerTemplate = '<div id="ng-tablePagerContainerDiv" style="text-align: center;padding-top: 5px;"' +
            ' class="ng-cloak ngtable-pager"> <div ng-if="params.settings().counts.length" ' +
            'class="ngtable-counts btn-group pull-right"> ' +
            '<button ng-repeat="count in params.settings().counts" type="button"' +
            ' ng-class="{\'active\':params.count()==count}" ng-click="params.count(count)" ' +
            'class="btn btn-default"> <span ng-bind="count"></span> </button> </div> ' +
            '<ul class="pagination ngtable-pagination ng-tablePagerUl">' +
            ...
            ' </li> </ul> </div> ';
        $templateCache.put('ngtable/pager.html',pagerTemplate);
    }]);
    return app;
}));